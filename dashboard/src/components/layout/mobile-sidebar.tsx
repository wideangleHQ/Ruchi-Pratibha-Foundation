      ...
      {isMobileOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed left-0 top-0 z-50 flex h-full w-72 flex-col bg-sidebar shadow-foundation-lg lg:hidden"
          >
            <div className="flex h-[var(--topbar-height)] items-center justify-between border-b px-4">
              <Link href="/" className="flex items-center gap-2.5">
                ...
                />
                <span className="font-cormorant text-sm font-semibold tracking-tight">Pratibha Foundation</span>
              </Link>
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        'flex items-center gap-3 rounded-[12px] px-3 py-2.5 font-manrope text-sm font-medium transition-colors',
                        isActive
                          ? 'bg-sidebar-accent text-sidebar-accent-foreground'
                          : 'text-sidebar-foreground hover:bg-sidebar-accent/50',
                      )}
                    >
                      <item.icon className={cn('h-4.5 w-4.5', isActive && 'text-sidebar-primary')} strokeWidth={1.75} />
                      {item.label}
                    </Link>
                  );
                })}
