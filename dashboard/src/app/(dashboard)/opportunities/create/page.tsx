...
import { createOpportunity, type OpportunityDetail } from '@/lib/api/opportunities';
...
function getInitialFormData(): FormData {
  return {
    ...
    coordinatorDesignation: null,
    coordinatorEmail: null,
    coordinatorMobile: null,
    coordinatorAltMobile: null,
    formConfig: { motivation: true, experience: true, skills: true, availability: true },
    requiredDocuments: [],
    galleryImageKeys: [],
    brochureKey: null,
    supportingDocKeys: [],
...
export default function CreateOpportunityPage() {
  ...
  const saveDraftMut = useMutation({
    mutationFn: () => createOpportunity({ ...form, opportunityStatus: 'DRAFT' }),
    onSuccess: (res) => {
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <PageHeader title="Create Opportunity" description="Fill in the details to create a new CSR opportunity." />
        </div>
                  ...
                  <div className="space-y-5">
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        ...
                        <Input id="regOpens" type="datetime-local" value={form.registrationOpens ?? ''} onChange={(e) => updateField('registrationOpens', e.target.value || null)} className="mt-1.5" />
                      </div>
                      <div>
                        <Label htmlFor="regCloses" className="font-manrope text-sm">Registration Closes</Label>
                        <Input id="regCloses" type="datetime-local" value={form.registrationCloses ?? ''} onChange={(e) => updateField('registrationCloses', e.target.value || null)} className="mt-1.5" />
                      </div>
                      <div>
                        <Label htmlFor="eventStart" className="font-manrope text-sm">Event Start Date</Label>
                        <Input id="eventStart" type="datetime-local" value={form.eventStartDate ?? ''} onChange={(e) => updateField('eventStartDate', e.target.value || null)} className="mt-1.5" />
                      </div>
                      <div>
                        <Label htmlFor="eventEnd" className="font-manrope text-sm">Event End Date</Label>
                        <Input id="eventEnd" type="datetime-local" value={form.eventEndDate ?? ''} onChange={(e) => updateField('eventEndDate', e.target.value || null)} className="mt-1.5" />
                      </div>
                      <div>
        ...
        <div className="flex items-center justify-between border-t pt-5">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => saveDraftMut.mutate()} disabled={saveDraftMut.isPending || createMut.isPending}>
