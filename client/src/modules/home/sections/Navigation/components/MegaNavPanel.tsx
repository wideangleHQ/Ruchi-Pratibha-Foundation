...
export const MegaNavPanel: React.FC<MegaNavPanelProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      {/* Inner Content Grid Aligned to Global Website Container with Refined Spacing */}
                ...
                <div className="flex flex-col gap-1.5 pt-2 border-t border-white/10">
                  {item.links.map((lnk) => (
                    <a
                      ...
                      className="group/link flex items-center justify-between py-0.5 text-xs font-manrope text-gray-300 hover:text-white transition-colors duration-150"
                    >
                      <span className="group-hover/link:translate-x-1 transition-transform duration-150">
                        {lnk.label}
                      </span>
                      <ArrowUpRight className="w-3 h-3 text-institutional-accent opacity-50 group-hover/link:opacity-100 group-hover/link:translate-x-0.5 group-hover/link:-translate-y-0.5 transition-all duration-150" />
                    </a>
                  ))}
                </div>
              </div>
