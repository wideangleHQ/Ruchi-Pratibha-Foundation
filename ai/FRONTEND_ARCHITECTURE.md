...
- **CSR Activities & Social Impact Archive (`app/csr/page.tsx`, `app/work/page.tsx`, `modules/csr/CSRPage.tsx`)**:
  - `CSRPage.tsx`: Living archive of compassion in action structured in 11 storytelling sections: `#csr-hero` (01. Hero - Creating Lasting Impact) → `#csr-philosophy` (02. Our CSR Philosophy - Serving Society with Purpose) → `#focus-areas` (03. 6 Interactive Archival Book Folio Cards) → `#csr-archive` (04. Searchable Activity Archive with Category Chips, Year/District Dropdowns & Detail Modal) → `#featured-csr` (05. Featured Initiatives - Mega Blood Donation, Green Odisha Plantation, Student Recognition) → `#stories-of-impact` (06. Human Impact Stories Carousel) → `#csr-timeline` (07. Service Timeline Epochs) → `#csr-partners` (08. Partners in Social Responsibility) → `#csr-gallery` (09. Filterable Visual Archive & Lightbox Modal) → `#csr-reports` (10. Transparency & Documentation Reports) → `#csr-cta` (11. Call to Action - Be Part of the Change).
  - `FocusAreas.tsx (Pillars of Action Archival Folio Cards)`: Configured 3D Archival Book/Folio interaction (`perspective: 1400px`, `transform-origin: left`). On Desktop/Laptop/Tablet, hovering or tapping a folio opens the cover (`rotateY(-108deg)`) with unhurried hardcover physics easing (`duration: 1200ms`, `cubic-bezier(0.22, 1, 0.36, 1)`), hinge resistance, and realistic momentum. On Mobile View (320px–480px), transforms into a full-bleed horizontal carousel (`w-[100vw] -ml-6 flex gap-4 overflow-x-auto overflow-y-hidden snap-x snap-mandatory pt-4 pb-6 touch-pan-x`) displaying ~20-25% peeks of adjacent cards (`w-[76vw] h-[460px]`) with zero vertical scrolling inside the track, auto-closing single open card state, and animated gold pagination dot indicators below.
  - `CSRArchive.tsx (Community Archive 2-Column Mobile Grid & Modal)`: Transformed mobile view (320px–480px) into a responsive 2-column grid gallery (`grid grid-cols-2 gap-3 sm:gap-4 block md:hidden`) featuring thematic image aspect boxes, category badges, dates, and locations. Tapping any card triggers an animated, scroll-locked event detail modal (`selectedActivity` with `document.body.style.overflow = 'hidden'`) displaying cover image, objectives list, activity tags, and close CTAs. Desktop grid (`hidden md:grid md:grid-cols-3 gap-8`) remains 100% unchanged.
  - `Mobile UX Refinement Pass (320px–480px)`: Replaced long vertical card stacks across `FeaturedCSR.tsx`, `CSRPartners.tsx`, `CSRGallery.tsx`, and `CSRReports.tsx` with full-bleed touch-first horizontal carousels (`w-[100vw] -ml-6 px-6 overflow-x-auto overflow-y-hidden snap-x snap-mandatory flex gap-4 touch-pan-x`) showing 15–20% peeks (`w-[78vw] shrink-0 snap-center`), active scroll tracking, and animated pagination dots. Enforced soft ambient borders (`shadow-none border border-black/10 dark:border-white/10`) to eliminate shadow bleeding or overlap artifacts, full-width stacked mobile CTAs in `CSRHero.tsx` & `CSRCTA.tsx`, and 44px+ minimum touch targets. Desktop, laptop, and tablet grid layouts remain 100% unchanged.
  - `CMS-Driven Data Architecture`: Entity definitions in `modules/csr/types/index.ts` (`CSRActivity`, `CSRCategory`, `CSRStory`, `CSRPartnerCategory`, `CSRReport`) and dataset in `modules/csr/data/csrData.ts` with explicit TODO markers for seamless future headless CMS integration.
  - `Route Aliasing`: Configured dual route entries in Next.js App Router (`/csr` and `/work`) so links and bookmarks resolve with zero 404s.
...
- **Dedicated Mobile Experience System (320px – 480px Viewports)**:
  ...
  - *Mobile Header Utility Actions & Compact Logo*: Mobile & Desktop Header (`Navigation/index.tsx`, `NavLogo.tsx`) integrates Search, Theme Toggle (Light/Dark), and Language Selector (`EN`/`OR`) beside the hamburger menu with touch-friendly targets, while `NavLogo.tsx` uses Next.js `Link` (`href="/"`) to seamlessly navigate visitors back to the homepage from any page.
