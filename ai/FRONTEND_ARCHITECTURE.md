...
- **About Foundation Page (`app/about/page.tsx`, `modules/about/AboutPage.tsx`)**:
  - `AboutPage.tsx`: Single-page editorial experience following the 7-step story sequence: `#about-hero` (Hero Section) → `#about-foundation` (About Foundation / Who We Are) → `#mission-vision` (Mission & Vision) → `#leadership` (Leadership) → `#timeline-archives` (Timeline Archives) → `#chairmans-message` (Chairman's Message) → `#governance-charter` (Governance Charter).
  - `useActiveSection.ts`: Custom `IntersectionObserver` hook tracking visible section IDs and dynamically highlighting the active section in both `MegaNavPanel.tsx` (Desktop) and `MobileMenu.tsx` (Mobile) with a gold pulsing indicator dot and gold typography state (`text-institutional-accent font-semibold`).
  - `Smooth In-Page Scroll & Header Offset`: Every section uses `scroll-mt-24 sm:scroll-mt-28` guaranteeing section headers remain fully visible below the fixed navigation bar. Clicking links or the `EXPLORE →` button automatically scrolls smoothly to the target section, updates URL hash, and closes the mobile navigation drawer without page jumps or reload.
